...
public class
 ...
 WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    private void notificationEvent(String ticker, Notification nf) {
        String text = ticker;
        int dex=ticker.charAt(ticker.length()-1);
    ...
    private void handleLuckyMoneyReceive() {
        ...
        if(event == Config.WX_AFTER_OPEN_HONGBAO) { //拆红包
            ...
            if (wechatVersion < USE_ID_MIN_VERSION) {
                ...
                if(targetNode == null) {
                    //分别对应固定金额的红包 拼手气红包
                    AccessibilityNodeInfo textNode = AccessibilityHelper.findNodeInfosByTexts(nodeInfo, "发了一个红包，金额随机");
